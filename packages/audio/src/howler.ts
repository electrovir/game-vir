...
(function () {
    ...
    /**
     * Create the global controller. All contained methods and properties apply to all sounds that
     * are currently playing or will be in the future.
     */
    ...
    HowlerGlobal.prototype = {
         ...
         * @returns {Howler/Float} Returns self or current volume.
         */
        volume: function (vol) {
            var self = this || Howler;
            vol = parseFloat(vol);
            ...
            if (typeof vol !== 'undefined' && vol >= 0 && vol <= 1) {
                ...
                for (var i = 0; i < self._howls.length; i++) {
                    if (!self._howls[i]._webAudio) {
                        ...
                        for (const id of ids) {
                            var sound = self._howls[i]._soundById(id);
        ...
        _setupCodecs: function () {
            ...
            var checkSafari = ua.includes('Safari') && !ua.includes('Chrome');
         ...
         * Attempt to automatically unlock audio on the first user interaction. Concept from:
        ...
        _unlockAudio: function () {
            ...
            if (!self._mobileUnloaded && self.ctx.sampleRate !== 44_100) {
            ...
            self._scratchBuffer = self.ctx.createBuffer(1, 1, 22_050);
        ...
        /**
         * Get an unlocked HTML5 Audio object from the pool. If none are left, return a new Audio
         * object and throw a warning.
         *
        ...
        /**
         * Automatically suspend the Web Audio AudioContext after no sound has played for 30
         * seconds. This saves processing/energy and fixes various browser-specific bugs with audio
         * getting stuck.
         *
         * @returns {Howler}
         */
    ...
    Howl.prototype = {
        ...
        init: function (o) {
            ...
            self._volume = o.volume === undefined ? 1 : o.volume;
            self._xhr = {
        ...
        pause: function (id) {
            ...
            for (const id_ of ids) {
                // Clear the end timer.
                self._clearTimer(id_);

                // Get the sound.
                var sound = self._soundById(id_);

                if (sound && !sound._paused) {
                    // Reset the seek position.
                    sound._seek = self.seek(id_);
                    sound._rateSeek = 0;
                    sound._paused = true;
        ...
        /**
         * Get/set the volume of this sound or of the Howl group. This method can optionally take 0,
         * 1 or 2 arguments. volume() -> Returns the group's volume value. volume(id) -> Returns the
         * sound id's current volume. volume(vol) -> Sets the volume of all sounds in this Howl
         * group. volume(vol, id) -> Sets the volume of passed sound id.
         *
        ...
        fade: function (from, to, len, id) {
            ...
            for (const id_ of ids) {
                ...
                if (sound) {
                    ...
                    self._startFadeInterval(sound, from, to, len, id_, typeof id === 'undefined');
                }
            }
        ...
        /**
         * Get/set the loop parameter on a sound. This method can optionally take 0, 1 or 2
         * arguments. loop() -> Returns the group's loop value. loop(id) -> Returns the sound id's
         * loop value. loop(loop) -> Sets the loop value for all sounds in this Howl group.
        ...
        /**
         * Get/set the playback rate of a sound. This method can optionally take 0, 1 or 2
         * arguments. rate() -> Returns the first sound node's current playback rate. rate(id) ->
         * Returns the sound id's current playback rate. rate(rate) -> Sets the playback rate of all
         * sounds in this Howl group. rate(rate, id) -> Sets the playback rate of passed sound id.
         *
            ...
            if (typeof rate === 'number') {
                ...
                for (const element of id) {
                    ...
                    if (sound) {
                        ...
                        if (self.playing(element)) {
                            sound._rateSeek = self.seek(element);
                        ...
                        if (self._endTimers[element] || !sound._paused) {
                            self._clearTimer(element);
                            self._endTimers[element] = setTimeout(
                                self._ended.bind(self, sound),
                                timeout,
                            );
                        }
        ...
        /**
         * Get/set the seek position of a sound. This method can optionally take 0, 1 or 2
         * arguments. seek() -> Returns the first sound node's current seek position. seek(id) ->
         * Returns the sound id's current seek position. seek(seek) -> Sets the seek position of the
         * first sound node. seek(seek, id) -> Sets the seek position of passed sound id.
         *
        ...
        /**
         * Unload and destroy the current Howl object. This will immediately stop all sound
         * instances attached to this group.
         */
        unload: function () {
            var self = this;
            ...
            for (i = 0; i < Howler._howls.length; i++) {
                if (
                    ...
                    self._src.includes(Howler._howls[i]._src)
                ) {
                    remCache = false;
                    break;
                }
        ...
        _cleanBuffer: function (node) {
            var self = this;
            var isIOS = Howler._navigator && Howler._navigator.vendor.includes('Apple');
